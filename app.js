...
function resetMovement(ask=true){if(ask&&!confirm("Clear current flight details?"))return;state.count=0;state.totals={passengers:0,highlift:0,crew:0};state.route={from:"",to:""};state.flight={number:"",airline:"",registration:""};state.timestamps={};state.editingId=null;persist();render()}
...
function exportCsv(){const h=store.get("abl_history",[]);if(state.loggedIn)h.unshift(state.session);const rows=[["Shift Start","Shift End","Bus","Start KM","End KM","Flight","Aircraft Registration","Airline","Movement","From","To","Passengers","Highlift","Crew","Timestamp Name","Timestamp"]];h.forEach(s=>s.movements.forEach(m=>m.timestamps.forEach(t=>rows.push([s.startedAt,s.endedAt||"",s.busId,s.startKm,s.endKm??"",m.flightNumber,m.aircraftRegistration||"",m.airline,m.type,m.from,m.to,m.passengers,m.highlift,m.crew,t.name,t.time]))));const csv=rows.map(r=>r.map(v=>`"${String(v??"").replace(/"/g,'""')}"`).join(",")).join("\n"),a=document.createElement("a");a.href=URL.createObjectURL(new Blob([csv],{type:"text/csv"}));a.download=`airside-bus-records-${new Date().toISOString().slice(0,10)}.csv`;a.click();URL.revokeObjectURL(a.href)}
...
function normalizeState(saved){
  ...
  saved.flight = saved.flight || {number:"",airline:"",registration:""};
